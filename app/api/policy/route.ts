...
export async function PUT(request: Request) {
  ...
  const next: PolicyConfig = {
    ...
    maxRetries: clampInt(body.maxRetries ?? current.maxRetries, 1, 8),
    maxMandateRetries: clampInt(body.maxMandateRetries ?? current.maxMandateRetries, 1, 5),
    recoveryWindowDays: clampInt(body.recoveryWindowDays ?? current.recoveryWindowDays, 1, 90),
    mandateRetryCooldownHours: clampInt(
      body.mandateRetryCooldownHours ?? current.mandateRetryCooldownHours,
      1,
      168,
    ),
